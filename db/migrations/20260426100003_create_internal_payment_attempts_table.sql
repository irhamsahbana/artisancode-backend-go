...
CREATE TABLE IF NOT EXISTS internal_payment_attempts (
    ...
    CONSTRAINT internal_payment_attempts_status_check CHECK (status IN ('initiated', 'pending', 'succeeded', 'failed', 'expired', 'cancelled')),
    CONSTRAINT internal_payment_attempts_amount_check CHECK (requested_amount >= 0 AND paid_amount >= 0)
);

CREATE INDEX IF NOT EXISTS idx_internal_payment_attempts_invoice_active
    ON internal_payment_attempts (internal_invoice_id, created_at DESC)
    WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_internal_payment_attempts_provider_request_active
    ON internal_payment_attempts (provider, provider_request_id)
    WHERE deleted_at IS NULL AND provider_request_id <> '';
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP TABLE IF EXISTS internal_payment_attempts;
-- +goose StatementEnd
