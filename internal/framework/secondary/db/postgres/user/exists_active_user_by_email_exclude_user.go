...
func (r *userRepo) ExistsActiveUserByEmailExcludeUser(ctx context.Context, email, excludeUserID string) (bool, error) {
	ctx, span := tracing.StartSpan(ctx, "internal:framework:secondary:db:postgres:user:exists_active_user_by_email_exclude_user:ExistsActiveUserByEmailExcludeUser")
	defer span.End()
