...
func (h *internalQuotationHandler) executeAction(c fiber.Ctx) error {
	...
	bundle, err := h.core.ExecuteQuotationAction(ctx, mapper.InternalQuotationActionFromRest(ctx, *req, c.Get("X-Request-ID")))
	if err != nil {
		code, errors := errmsg.Errors[error](ctx, err)
		return c.Status(code).JSON(response.Error(errors))
	}
	return c.Status(fiber.StatusOK).JSON(response.Success(mapper.InternalCommerceBundleToRest(*bundle), ""))
}
