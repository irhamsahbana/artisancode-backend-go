...
func (h *internalProductHandler) createInternalProductPricing(c *fiber.Ctx) error {
	...
	item, err := h.core.CreateInternalProductPricing(ctx, mapper.InternalProductPricingFromRestCreateToCore(ctx, *req))
	if err != nil {
		log.Ctx(ctx).Error().Err(err).Msg("Failed to create internal product pricing")
		code, errors := errmsg.Errors[error](ctx, err)
		return c.Status(code).JSON(response.Error(errors))
	}

	return c.Status(fiber.StatusCreated).JSON(response.Success(restentity.CreateInternalProductPricingResp{ID: item.ID}, ""))
}
