...
func TestLimitAuthenticationRequestReturnsTooManyRequests(t *testing.T) {
	...
	app.Get("/", func(c fiber.Ctx) error {
		if handled, err := h.limitAuthenticationRequest(c, "user-login", "BERUA", "owner@example.com"); err != nil || handled {
			return err
		}
...
func TestLimitRegistrationRequestAllowsRequest(t *testing.T) {
	h := &userHandler{
		rateLimiter: &stubAttemptLimiter{
	...
	app := fiber.New()
	app.Get("/", func(c fiber.Ctx) error {
		if handled, err := h.limitRegistrationRequest(c, "user-register", "BERUA", "owner@example.com"); err != nil || handled {
			return err
		}
		return c.SendStatus(fiber.StatusNoContent)
	})
