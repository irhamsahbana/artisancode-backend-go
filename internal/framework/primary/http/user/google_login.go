...
func (h *userHandler) googleLogin(c fiber.Ctx) error {
	tracedCtx, span := tracing.StartSpan(c.Context(), "internal:framework:primary:http:user:google_login:googleLogin")
	defer span.End()
	c.SetContext(tracedCtx)

	var (
		ctx = c.Context()
		req = new(restentity.GoogleLoginReq)
	...
	if handled, err := h.limitAuthenticationRequest(c, "user-google-login"); err != nil || handled {
		return err
	}
	...
	tokens, err := h.core.GoogleLogin(ctx, mapper.GoogleLoginReqToCore(ctx, *req))
	if err != nil {
		log.Ctx(ctx).Warn().Err(err).Any(common.LogKeyPayload, req.Log()).Msg("Google login service error")
