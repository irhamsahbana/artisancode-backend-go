...
func ForgotPasswordHandler(ctx context.Context, msg jetstream.Msg) {
	...
	if err != nil {
		...
		if err := msg.Ack(); err != nil {
			infraTracing.RecordError(span, err)
			log.Ctx(ctx).Error().Err(err).Any("payload", payload).Msg("consumer::ForgotPasswordHandler Error while rejecting message")
		}
		return
	}
